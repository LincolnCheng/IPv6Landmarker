...
iOS设备传输ARP，有时包含先前加入的WiFi网络的MAC地址（BSSID），如。 iSniff GPS会捕获这些ARP，并将MAC地址提交给Apple的WiFi定位服务（冒充iOS设备）以获取给定BSSID的GPS坐标。 如果仅针对特定设备捕获了SSID探针，则iSniff GPS可以查询Wiglele.net上的网络名称并可视化可能的位置。
通过地理定位多个SSID和WiFi路由器MAC地址，可以确定设备（并暗示其所有者）可能在哪里。
    '''
...
def main(args):
    ...
    #File of EUI-64 IPv6 address geolocations requested
        ...
        with open('landmark.csv',mode='w',encoding='utf-8', newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['IP','MAC','offset','lat','lon', 'improved'])
            with open(args.eui_file,'r',encoding='utf-8') as f:
                lines = f.readlines()
                ...
                #for num, line in tqdm.tqdm(enumerate(lines)):
                    ...
                    #if num<62980:  #中断后继续运行 276154、211871
                       #continue
                    ...
                    wide = 8
                    offset = 0
                    while -wide<=offset<= wide:
                        macInt = macToInt(mac)
                        new_bssid = intToMAC(macInt + offset)
                        if new_bssid == predictedBSSID:
                            improved = 0
                        else:
                            improved = 1
                        lat,lon = geolocate(new_bssid)
                        if lat != -180.0:
                            writer.writerow([ip, new_bssid, offset, lat, lon, improved])
                        if offset >= 0:     #从0向两边扩展
                            offset =-1-offset
                        else:
                            offset = - offset
